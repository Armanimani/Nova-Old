...
namespace nova::graphics
{
	Microsoft::WRL::ComPtr<IDXGISwapChain4> DX12SwapChainFactory::create(
		Microsoft::WRL::ComPtr<ID3D12CommandQueue>& command_queue, const HWND window_handle) noexcept
	{
		return create(command_queue, window_handle, 0, 0);
	}

	Microsoft::WRL::ComPtr<IDXGISwapChain4> DX12SwapChainFactory::create(
		Microsoft::WRL::ComPtr<ID3D12CommandQueue>& command_queue, const HWND window_handle, const UINT resolution_x, const UINT resolution_y) noexcept
	{
		Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain1;
		auto factory = DX12DXGIFactoryFactory::create();
		const auto descriptor = get_descriptor(resolution_x, resolution_y);
	...
	DXGI_SWAP_CHAIN_DESC1 DX12SwapChainFactory::get_descriptor(const UINT resolution_x, const UINT resolution_y) noexcept
	{
